...
public class RemoteRequirementTest {
  ...
  public void checkNeverTestForConsistencyWithRemote() {
    MockRemoteRequirement req = new MockRemoteRequirement(tsPath, RemoteRequirement.CHECK_NEVER);
    writeTSOnce(req, 6000L, true);
    req.setIsLocalAvailable(true);
  ...
  private boolean writeTSOnce(MockRemoteRequirement req, long ts, boolean isConsistentWithRemote) {
    req.setTs(ts);
  ...
  private long readTimestampFromFile(File file) {
    try {
      String persistentPathContent = FileCommands.readFileAsString(file);
