...
public class BuildCycleException extends RuntimeException {
  ...
  public BuildCycleException(String message, BuildRequest<?, ?, ?, ?> cycleCause, BuildCycle cycle) {
    ...
    assert cycle.getCycleComponents().contains(cycleCause) : "Cause " + cycleCause.createBuilder().description() + " not in cycle {" + cycle.getCycleComponents().stream().map(BuildRequest::createBuilder).map(Builder::description).reduce((String s1, String s2) -> s2 + " , " + s2).get() + "}";
